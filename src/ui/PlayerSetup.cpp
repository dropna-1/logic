    ...
    : on_start_(std::move(on_start))
{
    ...
    auto start_btn = Button("Continue", [this] {
        try {
    ...
    renderer_ = Renderer(container, [p1n=p1_name_input_, p1a=p1_age_input_, 
                                     p2n=p2_name_input_, p2a=p2_age_input_, start=start_btn] {
        return vbox({
            filler(),
            text("Player Setup") | bold | color(Color::Cyan) | center,
            separator(),
            ...
            start->Render() | center,
            filler()
        }) | borderRounded | color(Color::Cyan) | size(WIDTH, EQUAL, 60) | center;
    });
}
